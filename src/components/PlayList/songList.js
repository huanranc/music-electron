...
class SongList extends Component {
  ...
  componentDidMount() {
    ...
    fetch("/playlist/detail?id=" + this.props.match.params.id, myFetchOptions).then(response => {
      if (response.status !== 200) {
        throw new Error('未请求成功，状态码为' + response.status)
      }
      response
        .json()
        .then(json => json.playlist.tracks.map(item => {
          let newItem = {}
          newItem.id = item.id
          ...
          date.push(newItem)
          return this.setState({tracks: [date]})
        }))
        .catch(error => {
  ...
  // timeDt(time) {   let durationTime=parseInt(time/1000);   let minute =
  // parseInt(durationTime/60);   let second = durationTime%60+'';   let symbol =
  // ':';   if(minute == 0){       minute = '00';   }else if(minute < 10 ){
  // minute = '0'+minute;   }   if(second.length == 1){     second = '0'+second;
  ...
  // Song(id){   return id; } handleSong(id,e){   e.preventDefault();
  // this.setState({     songId:id   })   alert(id); }
  ...
  playAll = () => {
    if (this.state.tracks.length > 0) {
      this
        .props
        .changeCurrentSong(this.state.tracks[0][0]);
      this
        .props
        .setSongs(this.state.tracks);
    }
  ...
  collectSong(id) {
    return (e) => {
      ...
      fetch("/collection", myFetchOptions).then(response => {
        if (response.status !== 200) {
          throw new Error('未请求成功，状态码为' + response.status)
        }
        response
  ...
  render() {
    ...
    const songslist = tracks.length
      ? tracks[0].map((songs, index) => {
        // let arlist=songs.ar.length? songs.ar.map((ar,index) => {   ar.name }):'...'
        return <li
          key={index}
          ...
          : {}}>
          <span className="song-number">{index < 9
              ? `0${index + 1}`
              : index + 1
}
          </span>
          ...
          <a onClick={this.selectSong(songs, tracks[0])}>
            <i className="icon-play"></i>
          </a>
          {this.state.currenUserId !== undefined
            ? <Dropdown overlay={menu} trigger={['click']}>
                <a>
                  <i className="icon-add" onClick={this.collect(songs)}></i>
                </a>
              </Dropdown>
            : ''}
        ...
        <div className="list">
          <div className="song-list">
            <ul className="songlist">
              <li>
                <a className="bk-play-control" onClick={this.playAll}>
                  <i className="icon-play"></i>播放全部</a>
              </li>
              <li>
