...
class SongList extends Component {
  ...
  componentDidMount() {
    ...
    fetch("/playlist/detail?id=" + this.props.id, myFetchOptions).then(response => {
      ...
      response
        ...
        .then(json => {
          ...
          date.push(newItem)
          return this.setState({playlist: [date]})
        })
        .catch(error => {
  ...
  timeCreate(time) {
    ...
    let m = (unixTime.getMonth() + 1) > 10
      ? (unixTime.getMonth() + 1)
      : '0' + (unixTime.getMonth() + 1)
    let d = unixTime.getDate() > 10
      ? unixTime.getDate()
      : '0' + unixTime.getDate()
    return y + '-' + m + '-' + d
  }
          ...
          <div className="bk-name">
            ...
            <div className="bk-intr">
              <div className="intr-art-name">
                <img className="avatarUrl" alt="头像" src={song.avatarUrl}/>
                <Link to={`/artists/`}>{song.nickname}</Link>
              </div>
              <div className="intr-create-time intr-playlist-create-time">
                {this.timeCreate((song.createTime))}
                创建
              </div>
            </div>
