...
class User extends Component {
  ...
  deleList = (id, e) => {
    ...
    fetch('/user/list/del' + id, myFetchOptions).then(response => {
  ...
  //退出 http://localhost:3000/loginout
  loginout = () => {
    var myFetch = {
      method: 'GET',
      credentials: 'include',
    ...
    fetch("/loginout", myFetch).then(response => {
      if (response.status !== 200) {
        throw new Error('未请求成功，状态码为' + response.status)
      }
      response
        .json()
        .then(json => {
          this.setState({name: '未登录', currenUserId: json.user_id, songlist: ''})
        })
    })
  }
      ...
      ? this
        ...
        .map((list, index) => {
          return <li key={index} className="user-list">
            <NavLink to={`/userlist/${list.id}`}>{list.list_name}</NavLink>{index === 0
              ? ''
              : <i
                className="icon-delete createlist-dele"
                onClick={this
                .deleList
                .bind(this, list.id)}></i>}</li>
        })
      : '';
    ...
    return (
      <div className="user">
        ...
        <Login
          ...
          initusername={this.state.name}
          callbackstatus={this.changeStatus}
          initsonglist={this.state.changeList}
          callback={this.changeList}/>
        <CreateList
          createList={this.createList}
          create={this.state.create}
          newsonglist={this.state.changeList}
          callbacknew={this.changeList}/>
      </div>
    )
  };
}
