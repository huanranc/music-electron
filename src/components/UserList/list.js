...
class List extends Component {
  ...
  componentWillReceiveProps() {
    this.setState((prevState, props) => {
      ...
      fetch("/user/song/list", myFetchOptions).then(response => {
        ...
        response
          ...
          .then(json => json.result.map(item => {
            ...
            date.push(newItem)
            return this.setState({result: [date]})
          }))
          .catch(error => {
  ...
  playAll = () => {
    if (this.state.result.length > 0) {
      this
        .props
        .changeCurrentSong(this.state.result[0][0]);
      this
        .props
        .setSongs(this.state.result);
    }
  }
  ...
  deletSong = (id, e) => {
    ...
    fetch('/user/song/del' + id, myFetchOptions).then(response => {
      if (response.status !== 200) {
        throw new Error('未请求成功，状态码为' + response.status)
      }
      this.loadlists();
    ...
    const songslist = result.length > 0
      ? result[0].map((songs, index) => {
        return <li
          ...
          <a onClick={this.selectSong(songs, result[0])}>
