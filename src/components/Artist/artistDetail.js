...
class artistDetail extends Component {
    ...
    componentDidMount() {
        ...
        fetch("/artist/song?id="+ this.props.match.params.id, myFetchOptions)
        .then(response => {
        ...
        .then(response => {
            ...
            response.json().then(json => json.hotSongs.map(item => {
                    let newItem = {};
                    newItem.id = item.id
    ...
    fetch("/artist/album?id="+ this.props.match.params.id, myFetchOptions)
        .then(response => {
            if (response.status !== 200) {
                throw new Error('未请求成功，状态码为' + response.status)
            }
            response.json().then(json => json.hotAlbums.map(item => {
                    let newItem = {};
                    newItem.id = item.id
                    newItem.name = item.name
    ...
    render() {
        ...
        resultsong[0].map((newSong, index) => {
            return <li className="song" key={index} style={this.props.currentSong.id === newSong.id ? {color: "#6666ff"} : {}}>
                     <span>{
                        index < 9 ? `0${index + 1}` : index + 1
                    } </span>
                    <span className="song-name">{newSong.name}</span>
                    <a><i className="icon-play" onClick={this.selectSong(newSong, resultsong[0])}></i></a>
