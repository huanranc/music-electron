      ...
      if (response.status === 404) {
        ...
        response
          ...
          .then(json => {
            ...
            let userid = this.state.currenUserId !== 0
              ? this.state.currenUserId
              : '';
            if (userid !== undefined) {
              ...
              fetch("/user/person", myFetchOptions).then(response => {
                ...
                response
                  ...
                  .then(json => {
                    if (json.status === 200) {
                      ...
                      fetch("/user/list", myFetch).then(response => {
                        if (response.status !== 200) {
                          throw new Error('未请求成功，状态码为' + response.status)
                        }
                        response
                          ...
                          .then(json => {
                            ...
                            this
                              .props
                              .callbacknew(this.state.userlist)
  ...
  render() {
    return (
      <CSSTransition
        ...
        <div
          style={this.state.createList === true
          ? {
            display: "block"
          }
          : {
