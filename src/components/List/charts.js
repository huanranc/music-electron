...
class Charts extends Component {
  ...
  soar = () => {
    ...
    fetch("/toplist?idx=3", myFetchOptions).then(response => {
      ...
      response
        ...
        .then(json => json.playlist.tracks.map(item => {
          ...
          date.push(newItem)
          return this.setState({songs: [date], loading: false})
        }))
        .catch(error => {
          this.setState({songs: ''})
        })
  ...
  render() {
    ...
    const songlist = songs.length
      ? songs[0].map((nsong, index) => {
        return <li
          key={index}
          className="music-list-songs"
          onClick={this.selectSong(nsong, songs[0])}>
