...
class Lyric extends Component {
  ...
  render() {
    if (this.props.currentSong.id !== undefined) {
      if (this.currentSong.id !== this.props.currentSong.id) {
        ...
        fetch("/lyric?id=" + this.props.currentSong.id, myFetchOptions)
          ...
          .then(json => {
            this.setState({lrc: json.lrc})
          });
      }
    }
    const {lrc} = this.state;
    let lyric = lrc === undefined
      ? '纯音乐': lrc.lyric
    ...
    return <div className="singles-page">
      <div className="disk">
        <div
          className={"disk-inner" + (this.props.showSong
          ? " playing"
          : "")}>
          <img
            className="disk-img"
            alt="专辑图片"
            src={this.props.currentSong !== undefined
            ? this.props.currentSong.picUrl
            : ''}/>
      ...
      <div className="lyrics">
        ...
        <p className="lyrics-des">歌手：<Link to={`/artists/${this.props.currentSong.artId}`}>{this.props.currentSong.artName}</Link>
        </p>
        <p className="lyrics-des">专辑：<Link to={`/albums/${this.props.currentSong.alId}`}>{this.props.currentSong.alName}</Link>
        </p>
        <div className="lyrics-inner">
          <ul
            ...
            ? {}
            : {
              transform: `translateY(${ - 42 * (index - 4)}px)`
            }}>
            {lyric !== undefined
              ? lyrics.map((line, i) => <li
                key={`${i}`}
                style={index === i
                ? {
                  color: '#6666ff'
