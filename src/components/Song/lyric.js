        ...
        return <div className="singles-page">
            <div className="disk">
                <div className={"disk-inner" + (this.props.showSong ? " playing" : "")}>
                    <img className="disk-img" alt="专辑图片"
                         src={this.props.currentSong !== undefined ? this.props.currentSong.picUrl : ''} />
                    <span className="msk"></span>
                </div>
            </div>
            <div className="lyrics">
                <h3 className="lyrics-name">{this.props.currentSong.name}</h3>
                <p className="lyrics-des">歌手：<Link
                    to={`/artists/${this.props.currentSong.artId}`}>{this.props.currentSong.artName}</Link></p>
                <p className="lyrics-des">专辑：<Link
                    to={`/albums/${this.props.currentSong.alId}`}>{this.props.currentSong.alName}</Link></p>
                <div className="lyrics-inner">
                    <ul className="lyrics-list"
                        style={index === 0 ? {} : {transform: `translateY(${-42 * (index - 4)}px)`}}>
